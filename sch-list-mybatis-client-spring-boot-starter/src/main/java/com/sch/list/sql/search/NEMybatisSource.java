 ...
 * 不等搜索
 */
public class NEMybatisSource implements IMybatisSqlSource {
    ...
    public SqlNode doDeal(Configuration configuration, List<String> fieldName, String paramName) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            sql.append(MessageFormat.format(" {0} != #{1}", field, paramName));
            if(i != len - 1) {
                sql.append(" or ");
            }
        }
    ...
    public ListSearchConstant.SearchTypeEnum type() {
        return ListSearchConstant.SearchTypeEnum.NE;
    }
}
